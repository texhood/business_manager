...
const TwoFactorSettings = () => {
  ...
  const styles = {
    container: { padding: '24px', maxWidth: 520 },
  ...
  if (loading) return <div style={styles.container}><p>Loading 2FA settings…</p></div>;

  if (recoveryCodes) {
    return (
      <div style={styles.container}>
        <h3 style={styles.heading}>🔑 Recovery Codes</h3>
        <p style={styles.subtext}>Save these codes in a secure place. Each code can only be used once to sign in if you lose access to your authenticator app.</p>
        <div style={styles.recoveryBox}>
          {recoveryCodes.map((code, i) => <div key={i} style={styles.recoveryCode}>{code}</div>)}
        </div>
        <p style={styles.warning}>⚠ These codes will not be shown again. If you lose them, you can regenerate new ones from your security settings.</p>
        <button style={{ ...styles.btnPrimary, marginTop: 12 }} onClick={() => setRecoveryCodes(null)}>I've saved my codes</button>
      </div>
    );
  }
  ...
  if (setupData) {
    return (
      <div style={styles.container}>
        ...
        <p style={styles.subtext}>Scan the QR code below with your authenticator app (Google Authenticator, Authy, 1Password, etc.)</p>
        <div style={styles.qrContainer}><img src={setupData.qrCode} alt="TOTP QR Code" style={{ width: 200, height: 200 }} /></div>
        ...
        <form onSubmit={handleVerifySetup}>
          <div style={styles.formGroup}>
            ...
            <input id="verifyCode" type="text" inputMode="numeric" autoComplete="one-time-code" value={verifyCode} onChange={(e) => setVerifyCode(e.target.value.replace(/\D/g, ''))} placeholder="123456" maxLength={6} required autoFocus style={styles.codeInput} />
          </div>
          <div style={styles.actions}>
            <button type="submit" style={styles.btnPrimary} disabled={actionLoading || verifyCode.length !== 6}>{actionLoading ? 'Verifying…' : 'Verify & Enable'}</button>
            <button type="button" style={styles.btnSecondary} onClick={handleCancelSetup}>Cancel</button>
          </div>
        </form>
      </div>
    );
  ...
  if (showDisable) {
    return (
      <div style={styles.container}>
        ...
        <p style={styles.subtext}>Enter your password to confirm. This will remove 2FA protection from your account.</p>
        {error && <p style={styles.error}>{error}</p>}
        <form onSubmit={handleDisable}>
          <div style={styles.formGroup}>
            <label style={styles.label} htmlFor="disablePassword">Current Password</label>
            <input id="disablePassword" type="password" value={disablePassword} onChange={(e) => setDisablePassword(e.target.value)} required autoFocus style={styles.input} />
          </div>
          <div style={styles.actions}>
            <button type="submit" style={styles.btnDanger} disabled={actionLoading}>{actionLoading ? 'Disabling…' : 'Disable 2FA'}</button>
            <button type="button" style={styles.btnSecondary} onClick={() => { setShowDisable(false); setError(''); }}>Cancel</button>
          </div>
        </form>
      </div>
    );
  ...
  if (showRegenerate) {
    return (
      <div style={styles.container}>
        ...
        <p style={styles.subtext}>This will invalidate your existing recovery codes and generate new ones. Enter your password to confirm.</p>
        {error && <p style={styles.error}>{error}</p>}
        <form onSubmit={handleRegenerate}>
          <div style={styles.formGroup}>
            <label style={styles.label} htmlFor="regenPassword">Current Password</label>
            <input id="regenPassword" type="password" value={regenPassword} onChange={(e) => setRegenPassword(e.target.value)} required autoFocus style={styles.input} />
          </div>
          <div style={styles.actions}>
            <button type="submit" style={styles.btnPrimary} disabled={actionLoading}>{actionLoading ? 'Generating…' : 'Generate New Codes'}</button>
            <button type="button" style={styles.btnSecondary} onClick={() => { setShowRegenerate(false); setError(''); }}>Cancel</button>
          </div>
        </form>
      </div>
    );
  ...
  return (
    <div style={styles.container}>
      ...
      <p style={styles.subtext}>Add an extra layer of security to your account by requiring a code from an authenticator app when you sign in.</p>
      {error && <p style={styles.error}>{error}</p>}
      <div style={styles.card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div><strong>Status:</strong>{' '}<span style={styles.statusBadge(status?.enabled)}>{status?.enabled ? 'Enabled' : 'Disabled'}</span></div>
        </div>
      ...
      {status?.enabled ? (
        <div style={styles.actions}>
          <button style={styles.btnSecondary} onClick={() => setShowRegenerate(true)}>Regenerate Recovery Codes</button>
          <button style={styles.btnDanger} onClick={() => setShowDisable(true)}>Disable 2FA</button>
        </div>
      ) : (
        <button style={styles.btnPrimary} onClick={handleStartSetup} disabled={actionLoading}>{actionLoading ? 'Setting up…' : 'Enable Two-Factor Authentication'}</button>
      )}
    </div>
  );
};
