...
function JournalEntryForm({ accounts, classes, onSave, onCancel, editEntry }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validLines.length < 2) {
      setError('At least two valid lines are required');
      return;
    }
    
    if (!isBalanced) {
      setError(`Entry is not balanced. Debits: $${totalDebit.toFixed(2)}, Credits: $${totalCredit.toFixed(2)}`);
      return;
    }
    ...
    try {
      ...
      if (editEntry) {
        await journalEntriesService.update(editEntry.id, payload);
      } else {
        await journalEntriesService.create(payload);
      }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 20 }}>
          ...
          <div style={{ gridColumn: 'span 2' }}>
            ...
            <input
              ...
              placeholder="January depreciation, Year-end adjustment, etc."
              style={{
                width: '100%',
                padding: '8px 12px',
                borderRadius: 6,
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ backgroundColor: '#f9fafb' }}>
                <th style={{ padding: '10px 12px', textAlign: 'left', fontSize: 12, fontWeight: 600, borderBottom: '1px solid #e5e7eb' }}>Account</th>
                <th style={{ padding: '10px 12px', textAlign: 'left', fontSize: 12, fontWeight: 600, borderBottom: '1px solid #e5e7eb', width: '20%' }}>Description</th>
                <th style={{ padding: '10px 12px', textAlign: 'right', fontSize: 12, fontWeight: 600, borderBottom: '1px solid #e5e7eb', width: '12%' }}>Debit</th>
                <th style={{ padding: '10px 12px', textAlign: 'right', fontSize: 12, fontWeight: 600, borderBottom: '1px solid #e5e7eb', width: '12%' }}>Credit</th>
                <th style={{ padding: '10px 12px', textAlign: 'left', fontSize: 12, fontWeight: 600, borderBottom: '1px solid #e5e7eb', width: '15%' }}>Class</th>
              ...
              {lines.map((line, index) => (
                <tr key={index}>
                  ...
                  <td style={{ padding: '6px 4px' }}>
                    <select
                      value={line.class_id}
                      onChange={(e) => handleLineChange(index, 'class_id', e.target.value)}
                      style={{
                        width: '100%',
                        padding: '8px',
                        borderRadius: 4,
                  ...
                  <td style={{ padding: '6px 4px', textAlign: 'center' }}>
                    <button
                      ...
                      style={{
                        padding: '4px 8px',
                        backgroundColor: lines.length <= 2 ? '#f3f4f6' : '#fef2f2',
                        color: lines.length <= 2 ? '#9ca3af' : '#dc2626',
                        border: 'none',
                        borderRadius: 4,
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <button
              ...
              style={{
                padding: '10px 20px',
                backgroundColor: saving || !isBalanced ? '#9ca3af' : '#2563eb',
                color: 'white',
                border: 'none',
                borderRadius: 6,
...
export default function JournalEntriesView() {
  ...
  const handleEditEntry = async (entry) => {
    if (entry.status !== 'draft') {
      setMessage({ type: 'error', text: 'Only draft entries can be edited' });
      return;
    }
  ...
  const handlePostEntry = async (entry) => {
    if (!window.confirm(`Post entry ${entry.entry_number}? This will update account balances.`)) {
      return;
    }
  ...
  const handleDeleteEntry = async (entry) => {
    if (!window.confirm(`Delete draft entry ${entry.entry_number}?`)) {
      return;
    }
  ...
  const handleFormSave = () => {
    setShowForm(false);
    setEditEntry(null);
    setMessage({ type: 'success', text: 'Journal entry saved' });
    loadData();
  };
    ...
    <div style={{ padding: 24, maxWidth: 1400, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 16 }}>
              <thead>
                <tr style={{ backgroundColor: '#f9fafb' }}>
                  ...
                  <th style={{ padding: 10, textAlign: 'left', borderBottom: '1px solid #e5e7eb' }}>Description</th>
              ...
              <tbody>
                {viewEntry.lines?.map((line, i) => (
                  <tr key={i}>
                    ...
                    <td style={{ padding: 10, borderBottom: '1px solid #e5e7eb' }}>{line.description || '—'}</td>
                    <td style={{ padding: 10, textAlign: 'right', borderBottom: '1px solid #e5e7eb', fontFamily: 'monospace' }}>
              ...
              <tfoot>
                <tr style={{ fontWeight: 600, backgroundColor: '#f9fafb' }}>
                  <td colSpan={2} style={{ padding: 10, textAlign: 'right' }}>Totals:</td>
                  <td style={{ padding: 10, textAlign: 'right', fontFamily: 'monospace' }}>
                    ${parseFloat(viewEntry.total_debit).toFixed(2)}
                  </td>
                  <td style={{ padding: 10, textAlign: 'right', fontFamily: 'monospace' }}>
                    ${parseFloat(viewEntry.total_credit).toFixed(2)}
                  </td>
                  <td></td>
                </tr>
              </tfoot>
            ...
            <div style={{ fontSize: 12, color: '#6b7280' }}>
              Created by: {viewEntry.created_by_name || 'System'} | 
              {viewEntry.posted_at && ` Posted: ${new Date(viewEntry.posted_at).toLocaleString()}`}
              {viewEntry.voided_at && ` Voided: ${new Date(viewEntry.voided_at).toLocaleString()} - ${viewEntry.void_reason}`}
            </div>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ 
          ...
          <h3 style={{ margin: '0 0 8px', color: '#374151' }}>No Journal Entries</h3>
          <p style={{ margin: 0, color: '#6b7280' }}>
            Create your first journal entry to record depreciation, adjustments, or other non-cash transactions.
          </p>
        </div>
      ) : (
        <div style={{ backgroundColor: 'white', borderRadius: 8, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ backgroundColor: '#f9fafb' }}>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 500, fontSize: 14, borderBottom: '1px solid #e5e7eb' }}>Entry #</th>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 500, fontSize: 14, borderBottom: '1px solid #e5e7eb' }}>Date</th>
                <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 500, fontSize: 14, borderBottom: '1px solid #e5e7eb' }}>Description</th>
                <th style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 500, fontSize: 14, borderBottom: '1px solid #e5e7eb' }}>Amount</th>
                <th style={{ padding: '12px 16px', textAlign: 'center', fontWeight: 500, fontSize: 14, borderBottom: '1px solid #e5e7eb' }}>Status</th>
                <th style={{ padding: '12px 16px', textAlign: 'center', fontWeight: 500, fontSize: 14, borderBottom: '1px solid #e5e7eb' }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {entries.map((entry) => (
                <tr key={entry.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    ...
                    {entry.reference && <span style={{ color: '#6b7280', marginLeft: 8 }}>({entry.reference})</span>}
                  </td>
